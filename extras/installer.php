...
class DUPX_Bootstrap
{
    ...
    const ARCHIVE_FILENAME   = '20220905_blueprintwordpress_ef0bb7530e4c27907252_20220905220228_archive.zip';
    const ARCHIVE_SIZE       = '66015913';
    const INSTALLER_DIR_NAME = 'dup-installer';
    const PACKAGE_HASH       = 'ef0bb75-05220228';
    const SECONDARY_PACKAGE_HASH = '3729507-05220228';
    const VERSION            = '4.0.2';
    ...
    public function run()
    {
        ...
        $this->log('==DUPLICATOR INSTALLER BOOTSTRAP v4.0.2==');
        $this->log('----------------------------------------------------');
        ...
        if ($extract_installer) {
            ...
            if ($isZip) {
                ...
                try {
                    ...
                    @unlink($this->targetDupInst."/".$this->manualExtractFileName);
                }
                catch (Exception $ex) {
    ...
    private function extractInstallerShellexec($archive_filepath, $origDupInstFolder, $destination)
    {
        ...
        if ($unzip_filepath != null) {
            $unzip_command = "$unzip_filepath -q $archive_filepath ".$origDupInstFolder.'/* -d '.$destination.' -x '.$origDupInstFolder.'/'.$this->manualExtractFileName.' 2>&1';
    ...
    {
        if (($archive_filepath = filter_input(INPUT_GET, 'archive', FILTER_SANITIZE_STRING)) != false) {
...
<?php else : ?>
                ...
                <table cellspacing="0" class="header-wizard">
                    <tr>
                        ...
                        <td class="dupx-version">
                            ...
                            &raquo; <a target='_blank' href='dup-installer-bootlog__<?php echo DUPX_Bootstrap::SECONDARY_PACKAGE_HASH; ?>.txt'>dup-installer-bootlog__[HASH].txt</a>
                        </td>
                    </tr>
                </table>
    ...
    <?php echo $boot_error ?>
                        ...
                        <table class='settings'>
                            ...
                            <tr>
                                <td>Boot Log</td>
                                <td><a target='_blank' href='dup-installer-bootlog__<?php echo DUPX_Bootstrap::SECONDARY_PACKAGE_HASH; ?>.txt'>dup-installer-bootlog__[HASH].txt</a></td>
                            </tr>
                        </table>
                        <br/><br/>
